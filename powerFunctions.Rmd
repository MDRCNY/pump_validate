...
SS.blockedRCT.2<-function(M, numFalse, J, n.j, MDES, power, power.definition, MTP, marginError,
                            p, alpha, numCovar.1, numCovar.2=0, R2.1, R2.2, ICC, 
                            mod.type, sigma, omega,
                            tnum = 10000, snum=2, ncl=2) {
...
# We don't need snum high if not WY  
[1] 0.129725
[1] 0.1228725 0.1297250
[1] 0.4563
[1] 0.1262987
[1] 0.1262987 0.1297250
[1] 0.4096
[1] 0.1280119
> 
> 
...
NULL
> 
> test.SS <- SS.blockedRCT.2(M, numFalse = M, J=NULL, n.j, power=0.417, power.definition, MTP, marginError = 0.005,
+                           p, alpha, numCovar.1=0, numCovar.2=0, R2.1=r2, R2.2=0, ICC=0, 
+                           mod.type="constant", sigma=sigma, omega=NULL,
+                           tnum = 10000, snum=2, ncl=2)
Error in qt(p = (1 - alpha/2), df = t.df) : 
  if (!(MTP %in% c("WY-SS","WY-SD"))) snum <- 2
...
# Compute raw and BF MDES for INDIVIDUAL POWER
  crit.beta <- qt(power,df=t.df)
  multiplier.raw <- ifelse(power > 0.5, crit.alpha + crit.beta, crit.alpha - crit.beta) 
  multiplier.BF <- ifelse(power > 0.5, crit.alphaxM + crit.beta, crit.alphaxM - crit.beta)
...
# Compute J or n.j for raw and BF SS for INDIVIDUAL POWER
 if (doJ) {
   J.raw <- (multiplier.raw/MDES)^2 * ( (1-R2.1) / (p*(1-p)*n.j) )
   J.BF <-  (multiplier.BF/MDES)^2 * ( (1-R2.1) / (p*(1-p)*n.j) )
 }
 if (don.j) {
   n.j.raw <- ( (multiplier.raw/MDES)^2 * (1-R2.1) ) / (J *(p*(1-p)) )
   n.j.BF <- ( (multiplier.BF/MDES)^2 * (1-R2.1) ) / (J *(p*(1-p)) )
 }
...
### INDIVIDUAL POWER ###
if (power.definition=="indiv") {  
  if (MTP == "raw" & doJ) return(J.raw) 
  if (MTP == "BF" & doJ) return(J.BF) 
  if (MTP == "raw" & don.j) return(n.j.raw) 
  if (MTP == "BF" & don.j) return(n.j.BF) 
  }
...
# For minimal powers, makes starting value = raw
  if (power.definition != "indiv")  {
    ...
    while (ii < 10 & (target.power < power - marginError | target.power > power + marginError) ) {
      ...
      lowhigh.dist <- lowhigh[2]-lowhigh[1]
#      try.ss <- ifelse(is.over, lowhigh[2] - p.off * lowhigh.dist, lowhigh[1] + p.off*lowhigh.dist)
...
power.bira2c1(es=mdes1,alpha,two.tailed=TRUE,p,g1=numCovar.1,r21=r2,n=n.j,J=J)

sigma <-matrix(0.5,3,3)

test.MDES <- MDES.blockedRCT.2(M, numFalse = M, J, n.j, power=0.417, power.definition, MTP, marginError = 0.005,
                          p, alpha, numCovar.1=0, numCovar.2=0, R2.1=r2, R2.2=0, ICC=0, 
                          mod.type="constant", sigma=sigma, omega=NULL,
                          tnum = 10000, snum=2, ncl=2)
...
test.SS <- SS.blockedRCT.2(M, numFalse = M, J=NULL, n.j, MDES = power=0.417, power.definition, MTP, marginError = 0.005,
                          p, alpha, numCovar.1=0, numCovar.2=0, R2.1=r2, R2.2=0, ICC=0, 
                          mod.type="constant", sigma=sigma, omega=NULL,
                          tnum = 10000, snum=2, ncl=2)
...
test.power <- power.blockedRCT.2(M, MDES=rep(mdes1,3), J, n.j,
