...
- include an overview of main package functionality in introduction
- overall comment: right now, introduction focuses on the problem and methods, not on the package/solution. Doesn't necessarily feel like the intro for a software paper.
...
A frequent task for practitioners is to prospectively determine the statistical power of a study.
For many simple study designs and analyses, the researcher makes a small number of choices, such as the standard deviation of the outcome and the likely effect size, and then calculates the power using a well-known expression.
With complex study designs and analyses, this task is not always straightforward.
First, in complex settings there may not be a closed-form expression for power.
Second, the researcher must make a larger number of choices about the likely parameter values, and these parameters can substantially impact power.
A variety of software packages help users estimate power in different settings.
We extend the current available software with the power under multiplicity project (PUMP) package.
PUMP focuses on power estimation when a researcher wishes to determine the effect of an intervention on multiple outcomes in a multilevel randomized control trial (RCT).



A multilevel RCT is a setting in which units are nested in hierarchical groups.
This setting is common in education, in which students are nested within schools, which can be then nested within school districts.
...
In a single outcome scenario, the researcher must make a variety of choices for a multilevel RCT about the likely structure of the problem, such as the predictive power of covariates and the expected amount of variation.
These parameter choices impact the power of the study.

In a multiple outcome scenario, additional choices also impact the power.
One of the most important choices is the multiple testing procedure (MTP), which is how the $p$ values will be adjusted to compensate for multiple testing.
...
In this paper, we introduce the power under multiplicity project (PUMP) package.
PUMP allows users to estimate power for multilevel randomized control trials with multiple outcomes.
The contribution of this package is to properly calculate power when using a multiple testing procedure to adjust for multiple tests.
...
For example, even if a program would be considered successful should an effect of a specified size be found for at least one outcome, researchers may still want sufficient individual power because they want to know the probability of detecting effects on each particular outcome.
...
## Summary of the PUMP R Package

This paper and the R package we introduce, PUMP, focus on estimating power for multilevel randomized control trials (RCTs) that test a single intervention's impacts on a modest number of outcomes.
...
However, to our knowledge, no tools exist that take multiple outcomes into account.

The PUMP package fills this gap.
It allows users to estimate statistical power, sample size requirements, or minimum detectable effect sizes.
The estimates can be calculated for multiple definitions of statistical power, MTPs, and RCT designs and models.
The definitions of power are individual, complete, and relevant $d$-minimal powers, depending on the number of outcomes.
The MTPs are Bonferroni, Holm, Benjamini-Hochberg, and single-step and step-down versions of Westfall-Young.
The PUMP package supports RCT designs of 1, 2, or 3 levels, with randomization at any level.
In addition, users may make different modeling choices given a particular RCT design.
More detail about the supported RCT designs and models can be found in Section TODO.


<!-- KH comment: does frequentist need to be in the title?-->
...
In the frequentist framework, when framing impacts in terms of effect sizes, for the $m^{th}$ outcome, one typically tests a null hypothesis of no effect, $H_{0_m}: ES_m = 0$, against an alternative hypothesis $H_{1_m}: ES_m \neq 0$ for a two-sided tests or $H_{1_m}: ES_m > 0$ or $H1_m: ES_m < 0$ for a one-sided test.
...
<!-- KH COMMENT: also introduct raw = unadjusted-->
...
<!--KH COMMENT: have we defined $M$ yet?-->
...
<!-- KH comment: we don't actually say which MTPs provide weak or strong control, so I would either cut out that discussion, or clarify which ones do or don't-->
