...
\Plainauthor{Kristen Hunter \orcidlink{0000-0002-5678-4620}, Luke
Miratrix, Kristin Porter}
...
use of multiple testing procedures. We introduce the \pkg{PUMP}
\proglang{R} package as a tool for analysts to estimate statistical
...
for in two ways. First, power estimates from \pkg{PUMP} properly account
for the adjustment in \(p\) values from applying a multiple testing
procedure. Second, as researchers change their focus from one outcome to
multiple outcomes, different definitions of statistical power emerge.
\pkg{PUMP} allows researchers to consider a variety of definitions of
power, as some may be more appropriate for the goals of their study. The
package estimates power for frequentist multi-level mixed effects
models, and supports a variety of commonly-used RCT designs and models
and multiple testing procedures. In addition to the main functionality
of estimating power, minimum detectable effect size, and sample size
requirements, the package allows the user to easily explore sensitivity
of these quantities to changes in underlying assumptions.
}
...
Oakland, CA 94612-1900\\
  E-mail: \email{kristin.porter@mdrc.org}\\
...
\usepackage{amsmath} \usepackage{booktabs} \usepackage{orcidlink}
...
\begin{center}\includegraphics{jss_manuscript_files/figure-latex/plotsamplepowercurve-1} \end{center}
...
\begin{center}\includegraphics{jss_manuscript_files/figure-latex/othercorrections-1} \end{center}
...
\begin{center}\includegraphics{jss_manuscript_files/figure-latex/unnamed-chunk-11-1} \end{center}
...
\begin{center}\includegraphics{jss_manuscript_files/figure-latex/unnamed-chunk-13-1} \end{center}
...
\begin{center}\includegraphics{jss_manuscript_files/figure-latex/unnamed-chunk-15-1} \end{center}
...
We compute values from \pkg{PowerUpR} for individual, unadjusted power,
as \pkg{PowerUpR} only provides power estimates in that setting. For all
other types of power definitions and adjustments, we compare \pkg{PUMP}
...
To validate the estimates, we first check that the \pkg{PUMP} and
\pkg{PowerUpR} estimates match. In some settings we expect some
discrepancies between these values because \pkg{PUMP} has different
assumptions than \pkg{PowerUpR} for certain models. For details about
differences between \pkg{PUMP} and \pkg{PowerUpR} assumptions, see the
Technical Appendixs. Second, we check that the \pkg{PUMP} estimate is
...
validation. The green dots are \pkg{PUMP} estimate of power, the red dot
is the \pkg{PowerUpR} estimate of power, and the 95\% confidence
intervals based on the Monte Carlo simulations are shown in blue. To
validate that \pkg{PUMP} produces the expected result, we want to see
