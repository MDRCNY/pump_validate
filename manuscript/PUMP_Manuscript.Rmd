...
The authors of the `PUMP` package have also created a web application built with R Shiny. This web application calls the `PUMP` package and allows users to conduct calculations with a user-friendly interface, but it is less flexible than the package, with a focus on simpler scenarios (e.g., 10 or fewer outcomes). The app can be found at www.pumpmdrc.io.  
...
"These $M$ pairwise correlations are equal to the $M$ pairwise correlations between the residuals in the $M$ impact models.
...
For the models supported by `PUMP`, the relevant functions would be `lm()`, `lmer()` from the `lme4` library (@lme4), and `interacted_linear_estimators()` from the `blkvar` library.\footnote{This package is currently under development on GitHub; see https://github.com/lmiratrix/blkvar}
...
The main advantage of the `PUMP` package is to provide easily accessible estimation procedures so that users can properly account for power when making adjustments for multiple hypothesis testing. Users can also take advantage of the `PUMP` in an online interface at www.mdrcpump.io.
