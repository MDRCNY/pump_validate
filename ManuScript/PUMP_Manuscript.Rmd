...
<!-- KH: proposed new paragraph: brief overall intro before we dive in? -->
...
The package also allows for companion calculations of sample size and minimium detectable effect size.
Finally, the functionality allows users to easily explore power over a range of possible values of parameters, and visualize the results.
TODO: something about shiny?

<!-- KH comment: somewhere in intro, we need to make clear what setting we are in: multilevel RCTs-->

<!-- KH comment: we begin by discussing a bunch of things we will NOT solve, i.e. multiple subgroups, multiple points in time, etc. Can we motivate instead but just focus on multiple outcomes? In general I feel we can have a more exciting hook!-->
