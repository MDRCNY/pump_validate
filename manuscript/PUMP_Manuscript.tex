...
education and other social policy research.\footnote{Other options
  ...
  prevalent in applied research (for example, see Gelman A. (2012),
  Gelman A. (2007)).}
...
preferred'' (Porter (2018)).\footnote{The single-step and step-down
  ...
  pivotality (Ge (2003)).The distribution of the unadjusted test
...
power.\footnote{Complete power has also been referred to as
  ``conjunctive power'' (Bretz and Westfall (2011)) and ``all pairs
  power'' (Ramsey (1978)).} To calculate complete power, we do not need
to adjust the \(p\)-values, and can instead reject each individual test
...
favorable configuration (Chen (2011)). For example, consider if we have
four tests, with two false nulls and two true nulls. If we consider
...
probability of rejecting both null hypotheses) is \(\alpha^2\) (Deng,
Xu, and Wang (2008)).
...
correlations into account.\footnote{The Benjamini-Hochberg procedure was
  ...
  However, Benjamin (2001) showed that it also controls the FDR for true
  null hypotheses with ``positive regression dependence.'' This
  condition is satisfied for most applications in practice.} In
contrast, both of the Westfall-Young MTPs rely on the estimation of the
joint distribution of test statistics when the complete null hypothesis
(that there are not effects on any of the outcomes) is true. This joint
distribution of the test statistics is estimated from the study's data''
(Porter (2018)). For example, random permutations of the treatment
indicator break the association between treatment status and outcome.
Repeating these permutations a large number of times results in a
distribution of test statistics under the complete null. ``Because the
actual data are used to generate this null distribution, correlations
among the test statistics are captured. Then observed test statistics
can be compared with the distribution of test statistics under the
complete null hypothesis'' (Porter (2018)).\footnote{Instead of using
  test statistics, the Westfall-Young MTPs can alternatively compare raw
  \(p\)-values with the estimated joint null distribution of
  \(p\)-values.}
...
\item
  ...
  which at least \(d\) of the \(M\) hypotheses were rejected.\footnote{Note
    ...
    (e.g., Maurer and Mellein (1988); Chen (2011); T. Westfall Peter H
    and Wolfinger (2011)), ``disjunctive power'' (e.g., Bretz and
    Westfall (2011)), or ``any pair'' power (Ramsey (1978)). Chen (2011)
    use the terminology of ``r-power'' for what is referred to here as
    \(d-\)minimal power for \(d>1\).} For Diplomas Now, we could
  consider \(1\)-minimal power through \(4\)-minimal power.
\item
...
\NormalTok{grid}\SpecialCharTok{$}\NormalTok{ICC}\FloatTok{.3} \OtherTok{=} \FunctionTok{as.factor}\NormalTok{( grid}\SpecialCharTok{$}\NormalTok{ICC}\FloatTok{.3}\NormalTok{ )}
\NormalTok{grid }\OtherTok{=} \FunctionTok{filter}\NormalTok{( grid, MTP }\SpecialCharTok{==} \StringTok{"Holm"}\NormalTok{ )}
\FunctionTok{ggplot}\NormalTok{( grid, }\FunctionTok{aes}\NormalTok{( ICC}\FloatTok{.2}\NormalTok{, min1, }\AttributeTok{group =}\NormalTok{ ICC}\FloatTok{.3}\NormalTok{, }\AttributeTok{col =}\NormalTok{ ICC}\FloatTok{.3}\NormalTok{ ) ) }\SpecialCharTok{+}
...
Benjamin, \& Yekutieli. D., Y. 2001. {``The Control of the False
...
Bretz. Hothorn, F, and P. Westfall. 2011. {``Multiple Comparisons Using
r.''} Report.
...
Ge, Dudoit, Y. 2003. {``Resampling-Based Multiple Testing for Microarray
Data Analsysis.''} Journal Article. \emph{Test} 12: 1--77.

\leavevmode\vadjust pre{\hypertarget{ref-GelmanHill2007}{}}%
Gelman A., Yajima M., Hill J. 2007. \emph{Data Analysis Using Regression
...
Comparisons.''} Journal Article. \emph{Journal of Research on
...
Book Section. In \emph{Multiple Hypotheses Testing}, edited by G. Hommel
P. bauer and E. Sonnermann, 48--66. Heidelberg, Springer.
...
Comparisons.''} Journal Article. \emph{Journal of American Statistical
Association} 75: 479--87.
