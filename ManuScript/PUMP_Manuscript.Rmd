...
have $FWER=P(R>0)=E(\frac{B}{R})=FDR$. However,if any effects truly exist,then $FWER>=FDR$.
...
However, with multiple outcomes, finding such expressions can be quite difficult.
We want to avoid having to derive expressions for every possible design, model, MTP, and number of outcomes.
...
3. Use these test statistics to calculate $p$-values $p_1$.
4. Calculate power using the distribution of $p$-values.

However, we can simplify this process substantially by skipping the first step.
Given an assumed model and correlation structure for the test statistics, we can directly sample from $t_1$, the joint alternative distribution of the test statistics.

We now describe how to sample from $t_1$ directly.
...
For Westfall-Young procedures, this procedure is augmented by also sampling test statistics under the null distribution.
Under the alternative hypothesis, $t_m$ has a $t$ distribution with degrees of freedom $df$ and mean $\frac{\hat{ES}_m}{\hat{Q}_m}$; under the null hypothesis, $t_m$ has a $t$ distribution with degrees of freedom $df$ and mean $0$.

## RCT Designs

When designing a study, the researcher has two main choices:
1. The design of the experiment, including how many levels, and at which level randomization occurs.
2. The assumed model, including whether intercepts and treatment effects should be treated as constant, fixed, or random.

The PUMP package supports 1, 2, or 3 level models, with randomization occurring at any level.
...
The full design and model are specified by concantenating these together, e.g. d3.2_m3ff2rc.
...
Bonferroni is the most conservative correction, so it would result in the largest possible sample size, and thus Bonferroni provides our upper bound.
If we are interested in complete power, we have a larger upper bound; in order to have complete power of $0.8$, we would need each outcome to have an individual power of $(\text{target power}^(1/M))$.
On the other hand, our most conservative correction is doing no correction at all, and would result in the smallest possible sample size, so no adjustment provides our lower bound.
If we are interested in minimal power, we must have a smaller lower bound; in order to have 1-minimal power of $0.8$, each outcome needs to have individual power of $1 - (1 - \text{target power})^(1/M)$.

Second, given our lower and upper bounds, we now use optimization to find the sample size.
We first calculate power over a small grid of sample size points to estimate the functional form.
Then, we fit **TODO** a quadratic function to these points, which allows us to then quickly find the sample size.
**TODO** how much do I discuss corner cases?

## Validation

We completed extensive validation to ensure our power calculation procedure was correct.
...
In order to validate our estimates under multiplicity, we used a simulation approach.
...
A more detailed explanation of the validation procedure can be found in the Appendix, and full validation code and results
