...
Introduced by @BenjaminiHochberg1995, the FDR is the expected proportion of all rejected hypotheses that are erroneously rejected.
...
However, if the researchers are incorrect, the probability of detecting the extant effects can be diminished, sometimes substantially.
...
It is important to point out that under the assumption that some effects are truly null, we must change our notion of power for $d-$minimal powers (e.g., $1-$minimal power, $1/3-$minimal power, etc.) and complete power.
...
The ${M \choose 2}$ pairwise correlations between test statistics are equal to the $M$ pairwise correlations between the residuals in the $M$ regression models.
If there are no covariates in the regression models or if the $R^2$’s of the covariates are equivalent in all regression models, then the correlations between the test statistics are equal to the correlations between the outcomes.
However, having different $R^2$’s across the regression models reduces the correlations between the residuals and therefore between test statistics.\footnote{For example, one of the multiple outcomes may have a baseline covariate with a high $R^2$ while another may have a baseline covariate with
