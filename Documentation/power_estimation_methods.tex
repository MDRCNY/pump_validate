...
\end{itemize}
\item Whether the school-level and district-level treatment effects/slopes are:
\begin{itemize}
...
\subsection{Proposed naming conventions}

Proposed names would be:
levels\_treatment\_intercepts\_effects

Examples:
\begin{itemize}
\item 2level\_b2\_ir\_er: 2 level, blocked on the second level, random intercept, random treatment effect.  Corresponds to current blocked\_i1\_2r.
\item 3level\_b3c2\_if\_ec: 3 level, blocked on third level, clustered on second level, fixed intercepts, constant treatment effect. Corresponds to current blocked\_c2\_3f.
\end{itemize}
...
\begin{align*}
R_{3,m}^2 &= 1 - \frac{Var(w_{0,km})}{Var(\mu_{0,km})} \\
R_{2,m}^2 &= 1 - \frac{Var(u_{0,jkm})}{Var(\theta_{0,jkm} \mid D_{id})}\\
R^2_{1,m} &= 1 - \frac{Var(r_{ijkm})}{Var( Y_{ijkm}(0) \mid S_{id}, D_{id})}
\end{align*}
...
We next outline the R model and randomization scheme.

\textcolor{red}{Explain multiplier?}
...
We use overall variation, as we believe this definition is more natural.
Overall variation includes the student variation within each site, and also how the sites vary from each other.
Unfortunately, difference in how an effect size is defined can create some tension regarding some of the power formula.
In particular, with overall variation if we increase the ICC at level 2 or level 3, then there is less variation (relative to the reference variation) in level 1; thus an increased ICC will increase power for fixed effect regression as it is realizing the gains of a blocked experiment.
If effect size is relative to within-group variation, this gain is not seen.
We note how this plays out explicitly in the following sections.



%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Blocked individual randomization 2 level designs
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\newpage
\section{Blocked individual randomization, 2 level designs}
...
See \cite{Miratrix2020} for further, in-depth, discussion.
...
\subsection{Blocked randomization of individuals, 2 level design, constant effects (blocked\_i1\_2c)}
...
We can also call this a fixed effects, constant treatment model \cite{Miratrix2020}.
...
\begin{align}
J &= \left(\frac{M_{df}}{MDES}\right)^2 \left(\frac{(1-\text{ICC}_{2,m})(1-R^2_{1,m})}{\bar{n} \bar{T} (1 - \bar{T})} \right)\\
\bar{n} &= \left(\frac{M_{df}}{MDES}\right)^2 \left(\frac{(1-\text{ICC}_{2,m})(1-R^2_{1,m})}{J \bar{T} (1 - \bar{T})} \right) .
\end{align}
...
\item $\omega_{2,m} = 0$
\end{itemize}
Another way of viewing the PowerUp! formula without an ICC term is that we have
\begin{itemize}
\item $\text{ICC}_{2,m} = 0$
\end{itemize}

\textcolor{red}{Note that code currently reflects Powerup formula, not corrected formula.}
...
\newpage 
\subsection{Blocked randomization of individuals, 2 level design, fixed effects (blocked\_i1\_2f)}

\textbf{Characterization:} 2-level design, fixed intercepts, fixed slopes/treatment effects, no school covariates.
...
We can also call this a fixed effects with interactions model \cite{Miratrix2020}.
...
\textcolor{red}{For now, for validation, we continue to assume $\text{ICC}_{2,m} =0$}
...
\newpage 
\subsection{Blocked randomization of individuals, 2 level design, random effects (blocked\_i1\_2r)}

\textbf{Characterization:} 2-level design, random intercepts, random slopes/treatment effect, school covariates for intercept. Powerup also has school covariates for slope to allow for modeling treatment effect heterogeneity.
...
The model for estimating impacts on outcome $m$ is given by:
\begin{align}\label{eqn:bi12c_model}
Y_{ijkm} &= \psi_{1,ijkm} T_{ijk} + \theta_{0,jkm} + \sum_{p=1}^{g_{1,m}} \gamma_{mp} C_{ijkmp} + r_{ijkm}\\
...
The sample size formulas are:
\begin{align}
J &= \left(\frac{M_{df}}{MDES}\right)^2 \left(\text{ICC}_{2,m} \omega_{2,m} + \frac{(1-\text{ICC}_{2,m})(1-R^2_{1,m})}{\bar{T}(1 - \bar{T}) \bar{n}} \right)\\
\bar{n} &= \frac{(1-\text{ICC}_{2,m})(1-R^2_{1,m})}{\bar{T}(1 - \bar{T})\left(J \left(\frac{M_{df}}{MDES}\right)^{-2} - \text{ICC}_{2,m} \omega_{2,m}\right)}
\end{align}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Blocked individual randomization 3 level designs
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\newpage 
\section{Blocked randomization of individuals, 3 level designs}
...
Nesting in district does allow 
...
\subsection{Blocked randomization of individuals, 3 level design, random effects (blocked\_i1\_3r)}

\textbf{Characterization:} 3-level design, random intercepts for school and district, random slopes/treatment effect for school and district, school and district covariates for intercept. Powerup also allows for school and district covariates for cross-site impact heterogeneity.
...
\paragraph{Sample size formula.} 
\begin{align}
K &= \left(\frac{M_{df}}{MDES}\right)^2 \left(\text{ICC}_{3,m} \omega_{3,m} + \frac{\text{ICC}_{2,m} \omega_{2,m}}{J} + \frac{(1-\text{ICC}_{2,m}-\text{ICC}_{3,m})(1-R^2_{1,m})}{\bar{T}(1 - \bar{T}) J \bar{n}} \right)\\
J &= \frac{(1-\text{ICC}_{2,m}-\text{ICC}_{3,m})(1-R^2_{1,m}) + \bar{T}(1 - \bar{T}) \bar{n}\text{ICC}_{2,m} \omega_{2,m} + }{\bar{T}(1 - \bar{T}) \bar{n}\left(K \left(\frac{M_{df}}{MDES}\right)^{-2} - \text{ICC}_{3,m} \omega_{3,m}\right)}\\
\bar{n} &= \frac{(1-\text{ICC}_{2,m}-\text{ICC}_{3,m})(1-R^2_{1,m})}{\bar{T}(1 - \bar{T})\left(JK \left(\frac{M_{df}}{MDES}\right)^{-2} - J\text{ICC}_{3,m} \omega_{3,m} - \text{ICC}_{2,m} \omega_{2,m}\right)}
\end{align}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Cluster randomization 2 level designs
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\newpage 
\section{Cluster randomization, 2 level designs}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Simple cluster randomization, 2 level design, random effects 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\subsection{Simple cluster randomization, 2 level design, random effects (simple\_c2\_2r)}
...
\paragraph{Sample size formula.} 
\begin{align}
J &= \left(\frac{M_{df}}{MDES}\right)^2 \left(\frac{\text{ICC}_{2,m}(1-R^2_{2,m})}{\bar{T}(1- \bar{T})} + \frac{(1-\text{ICC}_{2,m})(1-R^2_{1,m})}{\bar{T}(1 - \bar{T}) \bar{n}} \right)\\
\bar{n} &= \frac{(1-\text{ICC}_{2,m})(1-R^2_{1,m})}{\bar{T}(1 - \bar{T}) J \left(\frac{M_{df}}{MDES}\right)^{-2} - \text{ICC}_{2,m}(1-R^2_{2,m})  }
\end{align}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Cluster randomization 3 level designs
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\newpage 
\section{Cluster randomization, 3 level designs}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Simple cluster randomization, 3 level design, random effects 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\subsection{Simple cluster randomization, 3 level design, random effects (simple\_c3\_3r)}

\textbf{Characterization:} 3-level design, random intercepts for schools and districts, constant treatment effect for all schools and districts, school and district covariates for intercept.
...
\paragraph{Sample size formula.} 
\begin{align}
K &= \left(\frac{M_{df}}{MDES}\right)^2 \left( \frac{\text{ICC}_{3,m}(1-R_{3,m}^2)}{\bar{T}(1 - \bar{T})}  + \frac{\text{ICC}_{2,m} (1-R_{2,m}^2)}{\bar{T}(1 - \bar{T}) J} + \frac{(1-\text{ICC}_{2,m}-\text{ICC}_{3,m})(1-R^2_{1,m})}{\bar{T}(1 - \bar{T}) J \bar{n}} \right)\\
J&=   \frac{\bar{n}\text{ICC}_{2,m} (1-R_{2,m}^2) + (1-\text{ICC}_{2,m}-\text{ICC}_{3,m})(1-R^2_{1,m})}{\bar{n}\left(\bar{T}(1 - \bar{T}) K \left(\frac{M_{df}}{MDES}\right)^{-2} - \text{ICC}_{3,m}(1-R_{3,m}^2)\right)} \\
\bar{n} &= \frac{(1-\text{ICC}_{2,m}-\text{ICC}_{3,m})(1-R^2_{1,m})}{\bar{T}(1 - \bar{T})JK \left(\frac{M_{df}}{MDES}\right)^{-2} - J\text{ICC}_{3,m}(1-R_{3,m}^2) - \text{ICC}_{2,m} (1-R_{2,m}^2)}
\end{align}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Blocked cluster randomization 3 level designs
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\newpage 
\section{Blocked cluster randomization, 3 level designs}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Blocked cluster randomization, 3 level design, fixed effects 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\subsection{Blocked cluster randomization, fixed effects (blocked\_c2\_3f)}

\textbf{Characterization:} 3-level design, random intercepts for schools, fixed intercepts for districts, fixed slopes/treatment effects for districts, constant slope/effects for schools within a district, school covariates for intercept.

The model for estimating impacts on outcome $m$ is given by:
\begin{align}\label{eqn:bi12c_model}
Y_{ijkm} &=  \theta_{0,jkm} + \sum_{p=1}^{g_{1,m}} \gamma_{mp} C_{ijkmp} + r_{ijkm}\\
\nonumber \theta_{0,jkm} &= \mu_{0,km} + \sum_{r=1}^{g_{2,m}} \delta_{mr} X_{jkmr} + u_{0,jkm}\\
\nonumber \mu_{0,km}  &= \Xi_{0,m} + \psi_{1,km} T_{k} + w_{0,km}\\
...
\frac{(1-\text{ICC}_{2,m})(1-R^2_{1,m})}{\bar{T}(1 - \bar{T}) J K\bar{n}} }.\end{align}
The degrees of freedom are
...
\textcolor{red}{Insert snarky comment about how probably ICC3 should not be zero here}
\textcolor{red}{Note that code currently reflects Powerup formula, not corrected formula.}

\paragraph{Sample size formula.} 
\begin{align}
K &= \left(\frac{M_{df}}{MDES}\right)^2 \left( \frac{\text{ICC}_{2,m} (1-R_{2,m}^2)}{\bar{T}(1 - \bar{T}) J} + \frac{(1-\text{ICC}_{2,m})(1-R^2_{1,m})}{\bar{T}(1 - \bar{T}) J \bar{n}} \right)\\
J &= \frac{\bar{n}\text{ICC}_{2,m} (1-R_{2,m}^2) + (1-\text{ICC}_{2,m})(1-R^2_{1,m})}{\bar{n} \bar{T}(1 - \bar{T})\left(\frac{M_{df}}{MDES}\right)^{-2} K } \\
\bar{n} &= \frac{(1-\text{ICC}_{2,m})(1-R^2_{1,m})}{\bar{T}(1 - \bar{T})J K \left(\frac{M_{df}}{MDES}\right)^{-2} -  \text{ICC}_{2,m} (1-R_{2,m}^2)}
\end{align}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newpage
\subsection{Blocked cluster randomization, random effects (blocked\_c2\_3r)}

\textbf{Characterization:} 3-level design, random intercepts for schools and districts, random slopes/treatment effect for districts, constant slope/effects for schools within a district, school and district covariates for intercept. Powerup also allows for district covariates for slope.
...
\paragraph{Sample size formula.} 
\begin{align}
K &= \left(\frac{M_{df}}{MDES}\right)^2 \left( \text{ICC}_{3,m} \omega_3  +  \frac{\text{ICC}_{2,m} (1-R_{2,m}^2)}{\bar{T}(1 - \bar{T}) J} + \frac{(1-\text{ICC}_{2,m}-\text{ICC}_{3,m})(1-R^2_{1,m})}{\bar{T}(1 - \bar{T}) J \bar{n}} \right)\\
J &=  \frac{\bar{n}\text{ICC}_{2,m} (1-R_{2,m}^2) + (1-\text{ICC}_{2,m}-\text{ICC}_{3,m})(1-R^2_{1,m})}{\bar{T}(1 - \bar{T})\left(\bar{n} K \left(\frac{M_{df}}{MDES}\right)^{-2} - \bar{n} \text{ICC}_{3,m} \omega_3\right)}\\
\bar{n} &= \frac{(1-\text{ICC}_{2,m}-\text{ICC}_{3,m})(1-R^2_{1,m})}{\bar{T}(1 - \bar{T})J \left(K \left(\frac{M_{df}}{MDES}\right)^{-2}  \text{ICC}_{3,m}\omega_{3,m}\right) -  \text{ICC}_{2,m} (1-R_{2,m}^2)}
\end{align}
