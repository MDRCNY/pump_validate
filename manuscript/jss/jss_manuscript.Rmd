...
The `PUMP` package allows for either one-sided or two-sided tests, but we proceed assuming two-sided tests going forward.^[For a one-sided test, depending on the direction of our alternative hypothesis, the raw $p$ value for test $m$ is computed as $p_m=\Prob(T_m \leq t_m)$ or $p_m=\Prob(T_m \geq t_m)$.]
...
```{r, echo = FALSE}
# kable(info$Parameters, format = 'latex', booktabs = TRUE, position = 'h!' ) %>%
#   kable_styling(position = "center")
```
