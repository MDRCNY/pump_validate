...
<!-- KH comment: we begin by discussing a bunch of things we will NOT solve, i.e. multile subgroups, multiple points in time, etc. Can we motivate instead but just focus on multiple outcomes? In general I feel we can have a more exciting hook!-->
...
Researchers typically worry that moving from one to multiple hypothesis tests, and thus employing MTPs, results in a loss of power. However, that pattern need not always be the case.
...
However, in studies with multiplicity, alternative definitions of power exist, and in some cases may be more appropriate (Chen, Luo, Liu, & Mehrotra, 2011; Dudoit, Shaffer, & Bodrick, 2003; Senn & Bretz, 2007; Westfall, Tobias, & Wolfinger, 2011).
...
<!-- KH comment: feels like this example should be earlier. Maybe this whole paragraph should be earlier!-->
...
<!-- KH comment: what type of research?  social and emotional development research?-->
<!-- KH comment: we need to check on whether JSS likes footnotes-->
For this type of research, the literature and tools for estimating statistical power, sample size, and minimum detectable effect size are extensive.^[For example, power estimating tools frequently used in social science research include @RN4473 Dong & Maynard, 2013; Hedges & Rhoads, 2010; Raudenbush et al., 2011; Spybrook et al., 2011).]
...
The package supports any of the following RCT designs and estimation models: 
...
In the frequentist framework, when framing impacts in terms of effect sizes, one typically tests a null hypothesis of no effect, $H_{0_m}: ES_m = 0$, against an alternative hypothesis of $H_{1_m}: ES_m \neq 0$ for a two-sided tests or $H_{1_m}: ES_m > 0$ or $H1_m: ES_m < 0$ for a one-sided test.
...
The quantity $\alpha$ is also referred to as the significance level. If $\alpha=0.05$, then the null hypothesis is rejected if the p-value is less than $0.05$, and it is concluded that the intervention had an effect because there is less than a $5$% chance that this finding is a false positive.
