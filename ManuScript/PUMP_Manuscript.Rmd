...
<!--For this type of research, the literature and tools for estimating statistical power, sample size, and minimum detectable effect size are extensive.^[For example, power estimating tools frequently used in social science research include @RN4473; @RN30153; @RN23884; @RN24179).]-->
...
<!--In the frequentist framework, when framing impacts in terms of effect sizes, one typically tests a null hypothesis of no effect, $H_{0_m}: ES_m = 0$, against an alternative hypothesis of a nonzero effect, $H_{1_m}: ES_m \neq 0$ for a two-sided tests or $H_{1_m}: ES_m > 0$ or $H1_m: ES_m < 0$ for a one-sided test.-->
...
Here, the term "raw" is used to distinguish this p-value from a p-value that has been adjusted for multiple hypothesis tests, as discussed below.
...
<!--Reducing the probability of false positives is the goal of MTPs.-->
...
<!--Therefore, MTPs generally reduce individual power compared with the situation when no multiplicity adjustments are made or the situation when there is only one hypothesis test.-->
...
<!-- MTPs also generally reduce power under all other definitions compared with the situation when no multiplicity adjustments are made -- but not necessarily compared with the situation when there is only one hypothesis test.-->
