...
With this package, researchers can estimate statistical power, minimum detectable effect size (MDES), and needed sample size for multi-level experimental designs, in which units are nested within hierarchical structures such as students nested within schools nested with school districts.
...
In addition to estimating power, the researcher can target a level of power and estimate either MDES or sample size requirements. Therefore, the package includes three core functions:
...
The Bonferroni and one of the Westfall-Young MTPs are singlestep; the Holm and Benjamini-Hochberg MTPs and the other Westfall-Young MTP are stepwise.
Note that stepwise procedures may be “step-down” or “step-upn,” referring to whether a procedure begins with the smallest $p$-value, and thus the largest effect size (step-down) or the largest $p$-value (step-up).
