...
assignment.vecs <- gen_simple_assignments(J = 20, K = 2, n.j = 100)

##### covariates
rho.D.scalar <- rho.X.scalar <- rho.C.scalar <- rho.default
...
rho.w.scalar <- rho.z.scalar <- rho.default
rho.u.scalar <- rho.v.scalar <-rho.default
rho.r.scalar <- rho.default
...
user.params.list <- list(
  M = 3
  , S.j = assignment.vecs[['S.j']]        # N-length vector of indiv school assignments i.e. (1,1,2,2,3,3)
  , S.k = assignment.vecs[['S.k']]        # N-length vector of indiv district assignments i.e. (1,1,1,2,2,2)
  , J = J                                 # number of districts
  , K = K                                 # number of schools
  , N = N                                 # number of individuals
  , n.j = n.j                             # number of individuals per school
  , rho.default = 0.5                     # default correlation value
  ################################################## level 3
...
pander::pandoc.table(
  ...
  paste0("PowerSimulation_", "rho_", rho.default,"_falseNull_", user.params.list[['M']]),
