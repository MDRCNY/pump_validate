...
if ( FALSE ) {
  ...
  samp
  table( samp$S.j )
  table( samp$S.j, samp$S.k )
  
  library( lme4 )
  M0 = lmer( Yobs_1 ~ 1 + D.ijk_1 + X.ijk_1 + C.ijk_1 + Tx + (1+Tx | S.j ),
             data = samp )
  summary( M0 )
...
one_run = function( model.params.list ) {
  ...
  # Randomize
  #T.ijk <- cluster_ra( samp.full$ID$S.j, prob = sim.params.list$p.j )
  T.ijk <- block_ra( samp.full$ID$S.j, prob = sim.params.list$p.j )
  
  samp.full$Yobs = gen_Yobs(samp.full, T.ijk)
  
  samp = gen_data_frame( samp.full, T.ijk )
  
  M0 = lmer( Yobs_1 ~ 1 + X.ijk_1 + C.ijk_1 + Tx * X.ijk_1 + (1 | S.j ),
             data = samp )
  summary( M0 )
  tst = as.data.frame( summary(M0)$coefficients[ c( "Tx", "X.ijk_1:Tx" ), ] )
  names( tst ) = c( "Est", "SE", "df", "t", "pvalue" )
  tst$Param = c("Tx","Tx:X" )
  tst$tau = mean( samp$Y1_1 - samp$Y0_1 )
...
design = tibble( psi = c( 0, 0.15, 0.3, 0.45, 0.6 ) )
design
