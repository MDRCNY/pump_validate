...
documentclass: jss
author:
  - name: Kristen Hunter~\orcidlink{0000-0002-5678-4620}
    affiliation: | 
       | Harvard University
...
For a two-sided test, the raw $p$-value for test $m$ is $p_m=2*Pr(T_m \geq |t_m|)$.
The `PUMP` package allows for either one-sided or two-sided tests, but we proceed assuming two-sided tests going forward.^[For a one-sided test, depending on the direction of our alternative hypothesis, the raw $p$-value for test $m$ is computed as $p_m=Pr(T_m \leq t_m)$ or $p_m=Pr(T_m \geq t_m)$.]
...
Individual power is $1 - Pr(\text{Type II error})$, so MTPs have the tradeoff of reducing Type I errors but also reducing individual power.
...
Therefore, the FWER is equivalent to $Pr(V > 0)$, the probability of at least one false positive finding.
...
For the models supported by `PUMP`, the relevant functions would be `lm()`, `lmer()` from the `lme4` library (@lme4), and `interacted_linear_estimators()` from the `blkvar` library.^[This package is currently under development on GitHub; see \url{https://github.com/lmiratrix/blkvar}]
...
```{r, echo = FALSE}
# info <- pump_info()
# kable( info$Design[,1:4], format = 'latex', booktabs = TRUE ) %>%
...
We introduce the power under multiplicity project (`PUMP`) `R` package, which estimates power for multi-level randomized control trials with multiple outcomes.
...
An online interface is also available at \url{https://mdrc.shinyapps.io/pump/}.
...
\section*{A. Package validation process}
